...
|---|---|---|
| **Clone RCE** — malicious repo runs build/test on clone/parse | Spoofing / Elevation | **Parse-only default:** tree-sitter extraction; no `npm install`, `go test`, or customer scripts. Indexer/compiler tiers (SCIP) were rejected — ADR 0017. Timeouts, file/size limits. |
...
- Browser → API (session)
- API → workers (internal service token)
